...
export default function SiteNav({ dark = true }: { dark?: boolean }) {
  ...
  return (
    <header className="site-header" style={{ position:"absolute",top:0,left:0,right:0,height:72,display:"flex",
      ...
      <nav className="site-nav-links" style={{ display:"flex",gap:24,fontFamily:"var(--font-ui)",fontSize:10.5,
        ...
        {NAV_LINKS.map(item => {
          ...
          return (
            <Link key={item.href} href={item.href} className={`nav-link-item${active ? " nav-link-active" : ""}`}
