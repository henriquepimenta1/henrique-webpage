 ...
 * Refatorado: tokens canônicos Field, rgba comentados, clamp() em headings
 */
...
/* Cor local de estrela — dado de UI, não token de sistema */
const STAR_COLOR = "#D4A64A";

/*
  catColors — dados de preset, não tokens de sistema.
  Plano item 07·PROBLEMA3: manter inline como const tipada, não mover para globals.
*/
...
function CountdownBanner() {
  ...
  return (
    <div style={{
      ...
      <span style={{ fontWeight: 700, fontSize: 16, letterSpacing: ".06em" }}>{m}:{s}</span>
  ...
  return (
    <div style={{ position: "relative" }}>
      <div style={{ overflow: "hidden" }}>
        <div style={{
          display: "flex",
          /* transform apenas — sem width/height como props animadas */
          transform: `translateX(-${idx * 100}%)`,
          transition: "transform .55s cubic-bezier(.4,0,.2,1)",
        }}>
          {TESTIMONIALS.map((t, i) => (
            <div key={i} style={{ minWidth: "100%", padding: "40px 48px", background: "var(--canvas)", border: "1px solid var(--line)", display: "flex", flexDirection: "column", gap: 16, boxSizing: "border-box" }}>
              <div style={{ color: STAR_COLOR, fontSize: 18, letterSpacing: 2 }}>★★★★★</div>
              <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: "clamp(17px, 2vw, 22px)", lineHeight: 1.6, color: "var(--bark)", margin: 0, maxWidth: "60ch" }}>
                "{t.text}"
              </p>
              <div style={{ display: "flex", alignItems: "center", gap: 14, marginTop: 4 }}>
                <div style={{ width: 44, height: 44, borderRadius: "50%", background: "var(--canvas-deep)", border: "1px solid var(--line)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "var(--font-ui)", fontWeight: 700, fontSize: 16, color: "var(--moss)" }}>
                  {t.name[0]}
                </div>
                <div>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 16, padding: "0 4px" }}>
        <div style={{ display: "flex", gap: 6 }}>
          {TESTIMONIALS.map((_, i) => (
            <button key={i} onClick={() => setIdx(i)} style={{
              /* transform de width via transition — OK (não é layout crítico, é UI dot) */
        ...
        <div style={{ display: "flex", gap: 8 }}>
          {(["←", "→"] as const).map((arrow, di) => (
            <button key={arrow} onClick={() => setIdx(i => (i + (di === 0 ? -1 : 1) + total) % total)} style={{ width: 36, height: 36, border: "1px solid var(--line)", background: "transparent", cursor: "pointer", fontFamily: "var(--font-mono)", fontSize: 14, color: "var(--bark)", display: "flex", alignItems: "center", justifyContent: "center" }}>{arrow}</button>
          ))}
        </div>
      </div>
    </div>
    ...
    <div style={{ borderTop: "1px solid var(--line)" }}>
      {items.map((item, i) => (
        <div key={i} style={{ borderBottom: "1px solid var(--line)" }}>
          ...
          {open === i && (
            <div style={{ paddingBottom: 22, fontFamily: "var(--font-serif)", fontSize: 16, lineHeight: 1.7, color: "var(--stone)", whiteSpace: "pre-line", maxWidth: "68ch" }}>
              {item.body}
            </div>
          )}
        </div>
...
function PresetCard({ preset, isActive, onSelect }: { preset: { key: string; name: string; desc: string; cat: string }; isActive: boolean; onSelect: () => void }) {
  ...
  return (
    <button onClick={onSelect} onMouseEnter={() => setHover(true)} onMouseLeave={() => setHover(false)} style={{
      background: "var(--canvas)", border: isActive ? "2px solid var(--rust)" : "1px solid var(--line)",
      overflow: "hidden", cursor: "pointer", padding: 0, textAlign: "left", position: "relative",
      /* transform apenas */
      transform: hover ? "translateY(-4px)" : "translateY(0)",
      transition: "transform .3s cubic-bezier(.2,.7,.2,1), border-color .2s",
    }}>
      <div style={{ position: "relative", aspectRatio: "1/1", overflow: "hidden" }}>
        <img src={`/images/presets/${preset.key}.jpg`} alt={preset.name} style={{
      ...
      <div style={{ padding: "14px 16px 16px" }}>
        <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: ".15em", color: "var(--stone)", marginBottom: 4, display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ display: "inline-block", width: 8, height: 8, borderRadius: "50%", background: CAT_COLORS[preset.cat] ?? "var(--rust)" }} />
          {preset.cat}
        </div>
        ...
        <div style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 12, color: "var(--stone)", lineHeight: 1.5 }}>{preset.desc}</div>
      </div>
    </button>
  );
}
...
function VsRow({ label }: { label: string }) {
  return (
    <>
      ...
      <div style={{ padding: "18px 24px", borderTop: "1px solid var(--line)", borderLeft: "1px solid var(--line)", textAlign: "center", fontFamily: "var(--font-mono)", fontSize: 18, color: "var(--rust)" }}>×</div>
      <div style={{ padding: "18px 24px", borderTop: "1px solid var(--line-dark)", background: "var(--forest)", color: "var(--rust-soft)", textAlign: "center", fontFamily: "var(--font-mono)", fontSize: 18, fontWeight: 700 }}>✓</div>
    </>
  );
}
...
function GuaranteeBadge() {
  return (
    <div style={{ display: "inline-flex", alignItems: "center", gap: 16, padding: "16px 24px", border: "1px solid var(--moss)", background: "rgba(74,88,56,.06)" }}>
  ...
  return (
    <div className="presets-lp" style={{ background: "var(--canvas)", color: "var(--bark)", fontFamily: "var(--font-ui)" }}>
      ...
      <section ref={heroRef} style={{ background: "var(--forest)", color: "var(--canvas)" }}>
        <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 0, minHeight: 720 }}>
          ...
          <div style={{ position: "relative" }}>
            ...
            {activePreset && (
              /* overlay de var(--forest) */
              <div style={{ position: "absolute", bottom: 56, left: 24, padding: "8px 14px", background: "rgba(30,42,24,.75)", backdropFilter: "blur(6px)", fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: ".2em", textTransform: "uppercase", color: "var(--ashe)" }}>
                {activePreset.name}
              </div>
            )}
          </div>
          ...
          <div style={{ padding: "64px 48px 56px", display: "flex", flexDirection: "column", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: ".24em", textTransform: "uppercase", color: "var(--ashe-dim)", marginBottom: 20 }}>
              ...
              <p style={{ fontFamily: "var(--font-serif)", fontSize: 16, lineHeight: 1.6, color: "var(--ashe-dim)", margin: "0 0 28px", maxWidth: "38ch" }}>
                A mesma cor que aparece no meu portfólio — expedições reais, luz natural, RAW direto da câmera.
              </p>

              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, paddingTop: 20, borderTop: "1px solid var(--line-dark)", marginBottom: 32 }}>
                {[
                  { k: "Formato",    v: ".xmp + .dng" },
                  ...
                  <div key={s.k}>
                    <div style={{ fontFamily: "var(--font-mono)", fontSize: 9, letterSpacing: ".22em", textTransform: "uppercase", color: "var(--ashe-dim)", marginBottom: 4 }}>{s.k}</div>
            ...
            <div>
              <div style={{ marginBottom: 20 }}>
                <div style={{ fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: ".18em", textTransform: "uppercase", color: "var(--ashe-dim)", marginBottom: 6 }}>
                  De R$ 119 por
                </div>
                <div style={{ display: "flex", alignItems: "flex-end", gap: 16, marginBottom: 4 }}>
                  <div style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(44px, 8vw, 72px)", fontWeight: 700, letterSpacing: "-.03em", lineHeight: 0.9 }}>
                    <span style={{ fontFamily: "var(--font-mono)", fontSize: 20, color: "var(--ashe-dim)", fontWeight: 400, marginRight: 4 }}>R$</span>
                    {PRICE_VISTA}
                  </div>
                  <div style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 14, color: "var(--ashe-dim)", paddingBottom: 8, lineHeight: 1.4 }}>
              ...
              <div style={{ marginTop: 20, display: "flex", alignItems: "center", gap: 10, fontFamily: "var(--font-mono)", fontSize: 10, letterSpacing: ".12em", color: "var(--ashe-dim)", textTransform: "uppercase" }}>
      ...
      <section style={{ padding: "96px 56px", background: "var(--canvas)" }}>
        <div style={{ maxWidth: 1360, margin: "0 auto" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 44, flexWrap: "wrap", gap: 24 }}>
            ...
            <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 16, color: "var(--stone)", maxWidth: "44ch", lineHeight: 1.55, margin: 0 }}>
      ...
      <section style={{ padding: "96px 56px", background: "var(--canvas-deep)", borderTop: "1px solid var(--line)" }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 style={{ fontFamily: "var(--font-ui)", fontWeight: 600, fontSize: "clamp(28px, 4vw, 44px)", letterSpacing: "-.02em", lineHeight: 1, margin: 0, color: "var(--bark)" }}>
              O que estão{" "}
      ...
      <section style={{ padding: "96px 56px", background: "var(--canvas)", borderTop: "1px solid var(--line)" }}>
        <div style={{ maxWidth: 1360, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(32px, 4vw, 48px)", fontWeight: 600, letterSpacing: "-.02em", lineHeight: 1, marginBottom: 48, marginTop: 0, color: "var(--bark)" }}>
            O que vem na{" "}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 40 }}>
            {[
              ...
              <div key={it.n} style={{ borderTop: "1px solid var(--line)", paddingTop: 22 }}>
                ...
                <div style={{ fontFamily: "var(--font-serif)", fontSize: 15, lineHeight: 1.65, color: "var(--stone)" }}>{it.d}</div>
              </div>
            ))}
          </div>
      ...
      <section style={{ padding: "96px 56px", background: "var(--canvas-deep)", borderTop: "1px solid var(--line)", borderBottom: "1px solid var(--line)" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto" }}>
          <h2 style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(28px, 3.5vw, 44px)", fontWeight: 600, letterSpacing: "-.02em", lineHeight: 1.05, marginBottom: 48, marginTop: 0, color: "var(--bark)", maxWidth: "22ch" }}>
            Por que não usar um{" "}
      ...
      <section style={{ padding: "96px 56px", background: "var(--forest)", color: "var(--canvas)" }}>
        <div style={{ maxWidth: 1360, margin: "0 auto" }}>
          <div style={{ marginBottom: 40, maxWidth: 720 }}>
            <h2 style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(32px, 4.5vw, 56px)", fontWeight: 600, letterSpacing: "-.02em", lineHeight: 1, margin: 0 }}>
              Névoa Suave —{" "}
            ...
            <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 18, color: "var(--ashe-dim)", marginTop: 14, lineHeight: 1.55 }}>
          ...
          {activePreset && (
            <div style={{ marginTop: 20, display: "flex", alignItems: "center", gap: 16, fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: ".18em", textTransform: "uppercase", color: "var(--ashe-dim)" }}>
              <span>Exibindo:</span>
      ...
      <section style={{ padding: "96px 56px", background: "var(--canvas)", display: "grid", gridTemplateColumns: "1fr 1.4fr", gap: 80, alignItems: "start" }}>
        <div>
          <h2 style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(32px, 4vw, 48px)", fontWeight: 600, letterSpacing: "-.02em", lineHeight: 1, margin: 0, color: "var(--bark)" }}>
            Tudo que você{" "}
            <span style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontWeight: 400, color: "var(--moss)" }}>precisa saber</span>.
          </h2>
          <p style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontSize: 16, color: "var(--stone)", marginTop: 20, lineHeight: 1.6, maxWidth: "32ch" }}>
      ...
      {/* ══════════════════════════════════════════════════
          10. CTA FINAL
      ══════════════════════════════════════════════════ */}
      <section style={{ padding: "112px 56px", background: "var(--forest)", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 80, alignItems: "center", borderTop: "1px solid var(--line-dark)" }}>
        <div>
          <h2 style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(44px, 6vw, 76px)", fontWeight: 700, letterSpacing: "-.03em", lineHeight: 0.94, margin: 0, color: "var(--canvas)" }}>
            Pronto pra<br />dar{" "}
            <span style={{ fontFamily: "var(--font-serif)", fontStyle: "italic", fontWeight: 400, color: "var(--rust-soft)" }}>cor</span>
            <br />às suas fotos?
          </h2>
          <p style={{ fontFamily: "var(--font-serif)", fontSize: 18, color: "var(--ashe-dim)", marginTop: 28, maxWidth: "38ch", lineHeight: 1.55 }}>
            Download imediato. Acesso vitalício. Garantia de 14 dias — se não curtir, devolvo sem pergunta.
          </p>
          <div style={{ marginTop: 32, display: "flex", alignItems: "center", gap: 10, fontFamily: "var(--font-mono)", fontSize: 11, letterSpacing: ".12em", color: "var(--ashe-dim)", textTransform: "uppercase" }}>
            <span style={{ color: "var(--rust-soft)", fontSize: 14 }}>✓</span>
            Garantia 14 dias · sem perguntas
          </div>
        </div>

        <div style={{ background: "var(--canvas)", padding: 40, border: "1px solid var(--line-dark)" }}>
          ...
          <div style={{ display: "flex", alignItems: "flex-end", gap: 16, marginBottom: 4 }}>
            <div style={{ fontFamily: "var(--font-ui)", fontSize: "clamp(44px, 8vw, 72px)", fontWeight: 700, letterSpacing: "-.03em", color: "var(--bark)", lineHeight: 1 }}>
          ...
          <div style={{ borderTop: "1px solid var(--line)", paddingTop: 20, marginBottom: 24 }}>
            {[
              ...
              <div key={item} style={{ display: "flex", gap: 12, padding: "8px 0", fontFamily: "var(--font-serif)", fontSize: 15, color: "var(--stone)" }}>
      ...
      <style>{`
        /*
          NOTA: --moss override removido.
          O override local .presets-lp { --moss: #8B6534 } sobrescrevia
          o token global --moss: #4A5838 com um ocre diferente.
          Se a intenção era um verde-oliva específico para esta página,
          criar token semântico: --moss-warm: #8B6534 em globals.css
          e usar aqui explicitamente.
        */
        ...
        @media (max-width: 768px) {
          .presets-lp [style*="position: fixed"] {
            flex-wrap: wrap; gap: 6px; padding: 8px 16px; font-size: 11px;
          }
          .presets-lp section { padding: 56px 20px !important; }
          .presets-lp section h1,
          .presets-lp section h2 { font-size: 36px !important; }
          .presets-lp [style*="repeat(4, 1fr)"] { grid-template-columns: repeat(2, 1fr) !important; }
          .presets-lp [style*="repeat(3, 1fr)"] { grid-template-columns: 1fr !important; }
          .presets-lp a[href*="cakto"] { width: 100%; text-align: center; box-sizing: border-box; }
          .presets-lp [style*="height: 560"] { height: 280px !important; }
        }

        @media (max-width: 480px) {
          .presets-lp section { padding: 40px 16px !important; }
          .presets-lp section h1,
          .presets-lp section h2 { font-size: 28px !important; }
          .presets-lp [style*="repeat(4, 1fr)"],
          .presets-lp [style*="repeat(2, 1fr)"] { grid-template-columns: 1fr !important; }
        }
      `}</style>
    </div>
  );
