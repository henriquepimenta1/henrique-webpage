...
const PRICE_VISTA = "39,90", PRICE_PARCEL = "5,19", PRICE_N = "9";
const CTA_URL = "https://pay.cakto.com.br/C4dmPFR";
const TOTAL_PRESETS = 45;

// ── Hero foto base (usada pelo "Try it" preview com filtros CSS)
const HERO_BASE = "public/images/presets/21-campo-seco-before.jpg";

// Helper: monta path das imagens reais do preset
const presetImg     = (k) => `public/images/presets/${k}.jpg`;
const presetImgRAW  = (k) => `public/images/presets/${k}-before.jpg`;

// Filtros CSS pra preview ao vivo no "Try It"
const PRESET_FILTERS = {
  "21-campo-seco":       "saturate(1.18) contrast(1.18) brightness(1.05) sepia(.18) hue-rotate(-12deg)",
  "17-nevoa-suave":      "saturate(.72) contrast(.94) brightness(1.10) hue-rotate(6deg)",
  "8-cinematico":        "saturate(.82) contrast(1.42) brightness(.90)",
  "2-bosque":            "saturate(1.22) contrast(1.18) brightness(.95) hue-rotate(8deg)",
  "6-dourado-reluzente": "saturate(1.32) contrast(1.18) brightness(1.10) sepia(.25) hue-rotate(-10deg)",
  "11-devaneio":         "saturate(.85) contrast(.95) brightness(1.10) hue-rotate(15deg) sepia(.10)",
  "19-oceano":           "saturate(1.28) contrast(1.20) brightness(.95) hue-rotate(8deg)",
  "22-poente":           "saturate(1.38) contrast(1.25) brightness(1.05) sepia(.22) hue-rotate(-16deg)",
  "1-explorador":        "saturate(.95) contrast(1.22) brightness(1) hue-rotate(-4deg)",
  "3-caminhante":        "saturate(.88) contrast(1.10) brightness(.95) hue-rotate(6deg)",
  "7-liberdade":         "saturate(1.18) contrast(1.18) brightness(1.08) sepia(.18)",
  "13-nostalgia":        "saturate(.70) contrast(1.28) brightness(.92) sepia(.30)",
};

const FALLBACK = "public/images/presets/21-campo-seco.jpg";

// Filtro fallback por categoria (pra presets sem filter customizado)
const CAT_FILTER_FALLBACK = {
  "Tom Verde":   "saturate(1.22) contrast(1.18) brightness(.97) hue-rotate(10deg)",
  "Tom Azul":    "saturate(.85) contrast(1.08) brightness(1.05) hue-rotate(8deg)",
  "Tom Laranja": "saturate(1.32) contrast(1.20) brightness(1.04) sepia(.20) hue-rotate(-12deg)",
  "Aesthetic":   "saturate(.92) contrast(1.18) brightness(1) sepia(.10)",
};

const CAT_COLORS = { "Tom Verde":"#7EC47E","Tom Azul":"#6FA3D8","Tom Laranja":"#D8924A","Aesthetic":"#C8905A" };
const PRESET_CATS = [
...
const TESTIMONIALS = [
  { name:"Mariana C.",  handle:"@mariana.foto",     text:"Apliquei no portfólio inteiro em uma tarde. A consistência entre as fotos é absurda.",        loc:"São Paulo, SP",        cover:"public/images/portfolio/lencois-aerial-drone.jpg" },
  { name:"Rafael T.",   handle:"@rafa_outdoor",     text:"Tentei mil presets grátis. Nenhum funciona em RAW de montanha. Esse funciona direto.",           loc:"Curitiba, PR",          cover:"public/images/portfolio/pico-parana-visto-do-topo-serradoibitiraquire.jpg" },
  { name:"Camila M.",   handle:"@camila.aventura",  text:"Em 10 minutos tava rodando no Lightroom Mobile. Recomendo demais.",                              loc:"Rio de Janeiro, RJ",    cover:"public/images/portfolio/ocaminhante-lencois.jpg" },
];
...
const vibe = (ms=8) => { try { navigator.vibrate?.(ms); } catch(e){} };

// ────────────────────────────────────────────────────────────────
// Countdown banner (igual antes — funciona bem)
// ────────────────────────────────────────────────────────────────
function CountdownBanner(){
  const KEY = "pf_deadline_v2";
  const deadline = _R.useMemo(()=>{
  ...
  const [secs, setSecs] = _R.useState(()=> Math.max(0, Math.floor((deadline - Date.now())/1000)));
  _R.useEffect(()=>{
  ...
  return (
    <div style={{position:"fixed",top:0,left:0,right:0,zIndex:100,background:"var(--rust)",color:"var(--canvas)",display:"flex",alignItems:"center",justifyContent:"center",gap:8,padding:"9px 12px",fontFamily:"var(--font-mono)",fontSize:11,letterSpacing:".08em",textTransform:"uppercase",flexWrap:"nowrap",overflow:"hidden"}}>
      ...
      <a href={CTA_URL} onClick={()=>vibe(12)} style={{padding:"4px 12px",background:"var(--canvas)",color:"var(--rust)",fontFamily:"var(--font-ui)",fontSize:11,fontWeight:700,letterSpacing:".1em",textDecoration:"none",textTransform:"uppercase",whiteSpace:"nowrap",marginLeft:4}}>Comprar →</a>
    </div>
  );
}

// ────────────────────────────────────────────────────────────────
// Sticky bottom CTA — aparece após 30% scroll (mobile principal-mente)
// ────────────────────────────────────────────────────────────────
function StickyCTA(){
  const [show, setShow] = _R.useState(false);
  _R.useEffect(()=>{
    const onScroll = ()=>{
  ...
  return (
    <div className="pf-sticky" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:90,background:"rgba(30,42,24,.96)",backdropFilter:"blur(10px)",WebkitBackdropFilter:"blur(10px)",borderTop:"1px solid rgba(232,223,201,.15)",transform:show?"translateY(0)":"translateY(100%)",transition:"transform .35s cubic-bezier(.4,0,.2,1)",padding:"10px 14px env(safe-area-inset-bottom, 10px)",display:"flex",alignItems:"center",gap:10}}>
      ...
      <a href={CTA_URL} onClick={()=>vibe(15)} style={{padding:"13px 18px",background:"var(--rust-soft)",color:"var(--forest)",fontFamily:"var(--font-ui)",fontSize:12,fontWeight:700,letterSpacing:".14em",textTransform:"uppercase",textDecoration:"none",flexShrink:0,display:"inline-flex",alignItems:"center",gap:6}}>
...
// ────────────────────────────────────────────────────────────────
// Skeleton image — pulsa enquanto carrega
// ────────────────────────────────────────────────────────────────
function SmartImage({ src, alt, style, filter, onClick }){
  const [loaded, setLoaded] = _R.useState(false);
  return (
    <div style={{position:"relative",width:"100%",height:"100%",overflow:"hidden",background:"#2a3820"}} onClick={onClick}>
      {!loaded && <div className="pf-skeleton" style={{position:"absolute",inset:0,background:"linear-gradient(90deg,#2a3820 0%,#3a4a2a 50%,#2a3820 100%)",backgroundSize:"200% 100%"}} />}
      <img src={src} alt={alt} draggable={false} onLoad={()=>setLoaded(true)} style={{width:"100%",height:"100%",objectFit:"cover",display:"block",opacity:loaded?1:0,transition:"opacity .35s ease",filter:filter||"none",...style}} />
    </div>
  );
}

// ────────────────────────────────────────────────────────────────
// BeforeAfter v2 — slider + tap to zoom + 3-way mode
// ────────────────────────────────────────────────────────────────
function BeforeAfter({ presetKey, aspectRatio="16/9", variant="section", allow3Way=false }){
  const [pos, setPos] = _R.useState(50);
  const [zoomed, setZoomed] = _R.useState(null); // {x,y} ou null
  const [mode3, setMode3] = _R.useState(false);
  const wrap = _R.useRef(null);
  const dragging = _R.useRef(false);
  const lastVibe = _R.useRef(0);

  const move = _R.useCallback((cx)=>{
    const el = wrap.current; if(!el) return;
    const r = el.getBoundingClientRect();
    const newPos = Math.max(2, Math.min(98, ((cx - r.left)/r.width)*100));
    setPos(newPos);
    // haptic ao chegar perto das bordas
    if((newPos<5 || newPos>95) && Date.now()-lastVibe.current>400){ vibe(6); lastVibe.current=Date.now(); }
  },[]);
  ...
  _R.useEffect(()=>{
    const el = wrap.current; if(!el) return;
    const onTM = (e)=>{ if(e.touches.length && dragging.current){ e.preventDefault(); move(e.touches[0].clientX); }};
  ...
  // Modo 3-way: 3 strips lado a lado (RAW real · Grátis exagerado · Outdoor C.)
  if(mode3 && allow3Way){
    return (
  ...
  return (
    <div ref={wrap}
      ...
      {/* RAW (left of slider) — imagem real -before.jpg do repo */}
...
// ────────────────────────────────────────────────────────────────
// Hero Carousel — 4 slides auto-rotating + swipe + dots
// ────────────────────────────────────────────────────────────────
const HERO_SLIDES = ["21-campo-seco","17-nevoa-suave","8-cinematico","6-dourado-reluzente"];
function HeroCarousel(){
  const [idx, setIdx] = _R.useState(0);
  const [paused, setPaused] = _R.useState(false);
  const total = HERO_SLIDES.length;
  const startX = _R.useRef(0);
  ...
  const activeName = PRESETS.find(p=>p.key===active)?.name || "—";

  return (
    <div style={{position:"relative",background:"var(--forest)",color:"var(--canvas)"}}
      onMouseEnter={()=>setPaused(true)} onMouseLeave={()=>setPaused(false)}>
      <div
        onTouchStart={e=>{ startX.current = e.touches[0].clientX; setPaused(true); }}
        onTouchEnd={e=>{
          const dx = e.changedTouches[0].clientX - startX.current;
          if(dx<-40){ setIdx(i=>(i+1)%total); vibe(8); }
          else if(dx>40){ setIdx(i=>(i-1+total)%total); vibe(8); }
          setTimeout(()=>setPaused(false), 2500);
        }}>
        <BeforeAfter presetKey={active} aspectRatio="4/3" variant="hero" allow3Way={true} />
      </div>

      {/* Nome do preset ativo */}
      <div style={{position:"absolute",bottom:12,left:12,padding:"5px 10px",background:"rgba(14,12,10,.78)",fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:".16em",textTransform:"uppercase",color:"rgba(232,223,201,.9)"}}>
        {activeName}
      </div>
      ...
      <div style={{position:"absolute",bottom:14,right:14,display:"flex",gap:6,zIndex:3}}>
        {HERO_SLIDES.map((_,i)=>(
          <button key={i} onClick={()=>{ setIdx(i); vibe(4); }} aria-label={`Slide ${i+1}`}
            style={{width:i===idx?22:7,height:7,borderRadius:4,border:"none",cursor:"pointer",background:i===idx?"var(--rust-soft)":"rgba(232,223,201,.4)",transition:"width .3s,background .3s",padding:0}} />
        ))}
      </div>
    </div>
  );
...
// ────────────────────────────────────────────────────────────────
// Specs accordion (chip "Detalhes técnicos ↓")
// ────────────────────────────────────────────────────────────────
function SpecsAccordion(){
  const [open, setOpen] = _R.useState(false);
  return (
    <div>
      <button onClick={()=>{ setOpen(o=>!o); vibe(6); }} style={{width:"100%",padding:"10px 14px",border:"1px solid rgba(232,223,201,.18)",background:open?"rgba(232,223,201,.06)":"transparent",color:"var(--canvas)",cursor:"pointer",fontFamily:"var(--font-mono)",fontSize:10,letterSpacing:".18em",textTransform:"uppercase",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span>Detalhes técnicos</span>
        <span style={{transform:open?"rotate(180deg)":"none",transition:"transform .25s",fontSize:11}}>▾</span>
      </button>
      <div style={{maxHeight:open?160:0,overflow:"hidden",transition:"max-height .3s cubic-bezier(.4,0,.2,1)"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,padding:"14px 4px 4px"}}>
          {[
            ["Formato",".xmp + .dng"],
            ["Presets","45 · 2 packs"],
            ["App","LR Classic, CC, Mobile"],
            ["Compat.","Camera Raw, PS"],
            ["Acesso","Vitalício"],
            ["Licença","Pessoal + comercial"],
          ].map(([k,v])=>(
            <div key={k}>
              <div style={{fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:".16em",textTransform:"uppercase",color:"rgba(232,223,201,.4)",marginBottom:2}}>{k}</div>
              <div style={{fontFamily:"var(--font-ui)",fontSize:12,fontWeight:600,color:"var(--canvas)"}}>{v}</div>
            </div>
          ))}
        </div>
      </div>
...
// Ordem dos chips intercala tons quentes ↔ frios pra impacto máximo
// ────────────────────────────────────────────────────────────────
const TRYIT_ORIGINAL = "public/images/presets/tryit-original.jpg";
const TRYIT_PRESETS = [
...
function TryItSection(){
  const [active, setActive] = _R.useState(TRYIT_PRESETS[0].key);
  const [pos, setPos] = _R.useState(50);
  const dragging = _R.useRef(false);
  ...
  return (
    <section style={{padding:"clamp(48px,7vw,80px) clamp(16px,5vw,48px)",background:"var(--canvas-deep)",borderBottom:"1px solid var(--line)"}}>
      <div style={{maxWidth:900,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:8,overflowX:"auto",overflowY:"hidden",paddingBottom:8,scrollSnapType:"x mandatory",WebkitOverflowScrolling:"touch"}} className="pf-chip-scroll">
          {TRYIT_PRESETS.map(p=>{
...
// ────────────────────────────────────────────────────────────────
// Stories testimonials — 9:16 com play, lightbox
// ────────────────────────────────────────────────────────────────
function StoriesTestimonials(){
  const [open, setOpen] = _R.useState(null); // index or null
  return (
    <section style={{padding:"clamp(48px,7vw,80px) clamp(16px,5vw,48px)",background:"var(--canvas)",borderBottom:"1px solid var(--line)"}}>
      <div style={{maxWidth:1000,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{fontFamily:"var(--font-hand)",fontSize:22,color:"var(--rust)",transform:"rotate(-1.5deg)",display:"inline-block",marginBottom:4}}>quem já usa—</div>
          <h2 style={{fontFamily:"var(--font-ui)",fontWeight:600,fontSize:"clamp(22px,4.5vw,38px)",letterSpacing:"-.02em",lineHeight:1,margin:0,color:"var(--bark)"}}>
            Pessoas reais, <span style={{fontFamily:"var(--font-serif)",fontStyle:"italic",fontWeight:400,color:"var(--moss)"}}>fotos reais</span>.
          </h2>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"clamp(8px,2vw,18px)"}} className="pf-stories">
          {TESTIMONIALS.map((t,i)=>(
            <button key={i} onClick={()=>{ setOpen(i); vibe(10); }} style={{position:"relative",aspectRatio:"9/16",overflow:"hidden",border:"none",padding:0,cursor:"pointer",background:"var(--forest)"}}>
              <SmartImage src={t.cover} alt={t.name} />
              <div style={{position:"absolute",inset:0,background:"linear-gradient(180deg,rgba(14,12,10,.05) 0%,rgba(14,12,10,.55) 60%,rgba(14,12,10,.92) 100%)"}} />
              {/* Avatar circle */}
              <div style={{position:"absolute",top:10,left:10,width:32,height:32,borderRadius:"50%",border:"2px solid var(--rust)",background:"var(--canvas)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-ui)",fontWeight:700,fontSize:13,color:"var(--bark)"}}>{t.name[0]}</div>
              {/* Play button */}
              <div style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",width:48,height:48,borderRadius:"50%",background:"rgba(232,223,201,.95)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 18px rgba(0,0,0,.4)"}}>
                <div style={{width:0,height:0,borderTop:"8px solid transparent",borderBottom:"8px solid transparent",borderLeft:"13px solid var(--bark)",marginLeft:3}} />
              </div>
              <div style={{position:"absolute",bottom:8,left:8,right:8,textAlign:"left"}}>
                <div style={{fontFamily:"var(--font-ui)",fontWeight:700,fontSize:11,color:"var(--canvas)",marginBottom:2}}>{t.name}</div>
                <div style={{fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:".12em",color:"rgba(232,223,201,.7)"}}>{t.handle}</div>
              </div>
              <div style={{position:"absolute",top:10,right:10,padding:"2px 6px",background:"rgba(14,12,10,.7)",color:"rgba(232,223,201,.85)",fontFamily:"var(--font-mono)",fontSize:7,letterSpacing:".1em",textTransform:"uppercase"}}>0:{15+i*7}</div>
            </button>
          ))}
        </div>
        <style>{`
          @media(max-width:640px){ .pf-stories{ grid-template-columns:1fr 1fr!important } .pf-stories > button:nth-child(3){ display:none } }
        `}</style>

        {open!==null && (() => {
          const t = TESTIMONIALS[open];
          return (
            <div onClick={()=>{ setOpen(null); vibe(6); }} style={{position:"fixed",inset:0,zIndex:200,background:"rgba(14,12,10,.95)",display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)"}}>
              <div onClick={e=>e.stopPropagation()} style={{position:"relative",maxWidth:380,width:"100%",aspectRatio:"9/16",background:"var(--forest)",overflow:"hidden",boxShadow:"0 20px 60px rgba(0,0,0,.6)"}}>
                <img src={t.cover} alt={t.name} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover"}} />
                <div style={{position:"absolute",inset:0,background:"linear-gradient(180deg,rgba(14,12,10,.3) 0%,rgba(14,12,10,.4) 50%,rgba(14,12,10,.95) 100%)"}} />
                {/* Progress bars */}
                <div style={{position:"absolute",top:10,left:10,right:10,display:"flex",gap:4}}>
                  {TESTIMONIALS.map((_,i)=>(<div key={i} style={{flex:1,height:2,background:i<=open?"var(--canvas)":"rgba(232,223,201,.3)"}} />))}
                </div>
                {/* Header */}
                <div style={{position:"absolute",top:24,left:14,right:14,display:"flex",alignItems:"center",gap:10}}>
                  <div style={{width:38,height:38,borderRadius:"50%",border:"2px solid var(--rust)",background:"var(--canvas)",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"var(--font-ui)",fontWeight:700,fontSize:15,color:"var(--bark)"}}>{t.name[0]}</div>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontFamily:"var(--font-ui)",fontWeight:700,fontSize:13,color:"var(--canvas)"}}>{t.name}</div>
                    <div style={{fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:".1em",color:"rgba(232,223,201,.65)"}}>{t.handle} · {t.loc}</div>
                  </div>
                  <button onClick={()=>{ setOpen(null); vibe(6); }} style={{width:30,height:30,borderRadius:"50%",border:"1px solid rgba(232,223,201,.4)",background:"transparent",color:"var(--canvas)",fontFamily:"var(--font-mono)",fontSize:14,cursor:"pointer",padding:0,display:"flex",alignItems:"center",justifyContent:"center"}}>×</button>
                </div>
                {/* Quote */}
                <div style={{position:"absolute",bottom:48,left:18,right:18}}>
                  <div style={{color:"#D4A64A",fontSize:15,marginBottom:8}}>★★★★★</div>
                  <p style={{fontFamily:"var(--font-serif)",fontStyle:"italic",fontSize:19,lineHeight:1.45,color:"var(--canvas)",margin:0,textShadow:"0 2px 12px rgba(0,0,0,.5)"}}>"{t.text}"</p>
                </div>
                {/* Nav */}
                <button onClick={()=>{ setOpen((open-1+TESTIMONIALS.length)%TESTIMONIALS.length); vibe(6); }} style={{position:"absolute",left:0,top:60,bottom:60,width:"30%",background:"transparent",border:"none",cursor:"pointer",color:"rgba(255,255,255,0)"}}>‹</button>
                <button onClick={()=>{ setOpen((open+1)%TESTIMONIALS.length); vibe(6); }} style={{position:"absolute",right:0,top:60,bottom:60,width:"30%",background:"transparent",border:"none",cursor:"pointer",color:"rgba(255,255,255,0)"}}>›</button>
                <div style={{position:"absolute",bottom:14,left:0,right:0,textAlign:"center",fontFamily:"var(--font-mono)",fontSize:8,letterSpacing:".18em",textTransform:"uppercase",color:"rgba(232,223,201,.5)"}}>Toque ‹ › pra navegar</div>
              </div>
            </div>
          );
        })()}
    ...
    <section style={{padding:"clamp(48px,7vw,80px) clamp(16px,5vw,48px)",background:"var(--canvas-deep)",borderBottom:"1px solid var(--line)"}}>
      <div style={{maxWidth:700,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"center",gap:6,marginTop:14}}>
          {VS_ROWS.map((_,i)=>(<button key={i} onClick={()=>{ setIdx(i); vibe(6); }} style={{width:i===idx?22:7,height:7,borderRadius:4,border:"none",cursor:"pointer",background:i===idx?"var(--rust)":"var(--line)",transition:"width .3s,background .3s",padding:0}} />))}
        </div>
      </div>
    </section>
  );
...
// ────────────────────────────────────────────────────────────────
function FaqChat(){
  const [revealed, setRevealed] = _R.useState([0]); // start with first revealed
  const reveal = (i)=>{
...
// CTA Button (reusável)
// ────────────────────────────────────────────────────────────────
function CTAButton({ label="Comprar" }){
  return (
    <a href={CTA_URL} onClick={()=>vibe(15)} className="pf-cta" style={{display:"flex",flexDirection:"column",alignItems:"stretch",gap:5,textDecoration:"none",width:"100%",boxSizing:"border-box"}}>
...
// MAIN PAGE
// ────────────────────────────────────────────────────────────────
function PresetsFotografiaPage(){
  return (
    <div className="pf-lp" style={{background:"var(--canvas)",color:"var(--bark)",fontFamily:"var(--font-ui)",overflowX:"hidden",maxWidth:"100vw",paddingBottom:88}}>
      ...
      <div style={{paddingTop:40}}>
        <header style={{height:56,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 clamp(16px,5vw,40px)",borderBottom:"1px solid rgba(42,33,26,.1)"}}>
          ...
          <Link href="/presets" style={{fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:".2em",textTransform:"uppercase",color:"var(--stone)",textDecoration:"none"}}>← Presets</Link>
        </header>
      </div>
...
// Compact collection list (separado pra manter MAIN limpo)
function CompactCollection(){
  const [openKey, setOpenKey] = _R.useState(null);
      ...
      {grouped.map(({cat,items})=>(
        <div key={cat.id}>
          <div style={{padding:"8px 0 6px",display:"flex",alignItems:"center",gap:8,borderBottom:"2px solid var(--line)",marginTop:4}}>
            <span style={{display:"inline-block",width:7,height:7,borderRadius:"50%",background:CAT_COLORS[cat.label]||"var(--rust)"}} />
            <span style={{fontFamily:"var(--font-mono)",fontSize:9,letterSpacing:".18em",textTransform:"uppercase",color:"var(--stone)"}}>{cat.label} · {cat.count}</span>
          </div>
          {items.map(p=>{
            const open = openKey===p.key;
            return (
              <div key={p.key}>
                <button onClick={()=>{ setOpenKey(open?null:p.key); vibe(6); }} style={{width:"100%",display:"grid",gridTemplateColumns:"44px 1fr 26px",alignItems:"center",gap:10,padding:"11px 0",background:open?"rgba(166,84,43,.05)":"transparent",border:"none",borderBottom:"1px solid var(--line)",cursor:"pointer",textAlign:"left"}}>
                  <div style={{width:44,height:44,overflow:"hidden",border:open?"2px solid var(--rust)":"1px solid var(--line)",flexShrink:0}}>
                    <img src={presetImg(p.key)} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",display:"block"}} />
                  </div>
                  <div style={{minWidth:0}}>
                    <div style={{fontFamily:"var(--font-ui)",fontWeight:600,fontSize:14,color:"var(--bark)",lineHeight:1.2,marginBottom:2}}>{p.name}</div>
                    <div style={{fontFamily:"var(--font-serif)",fontStyle:"italic",fontSize:12,color:"var(--stone)",lineHeight:1.3}}>{p.desc}</div>
                  </div>
                  <div style={{width:24,height:24,border:`1px solid ${open?"var(--rust)":"var(--line)"}`,background:open?"var(--rust)":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    <span style={{fontFamily:"var(--font-mono)",fontSize:14,color:open?"var(--canvas)":"var(--stone)",transform:open?"rotate(45deg)":"none",transition:"transform .25s",lineHeight:1}}>+</span>
                  </div>
                </button>
                {open && (
                  <div style={{padding:"12px 0 16px",borderBottom:"1px solid var(--line)",background:"rgba(166,84,43,.03)"}}>
                    <BeforeAfter presetKey={p.key} aspectRatio="16/9" variant="inline" />
                  </div>
                )}
              </div>
            );
...
function MasterDetailCollection(){
  ...
  const [open, setOpen] = _R.useState(()=>({ "Tom Verde":true, "Tom Azul":true, "Tom Laranja":false, "Aesthetic":false }));
  const [sel, setSel] = _R.useState(allItems[0]?.key);
  const [pos, setPos] = _R.useState(52);
  const [rawOk, setRawOk] = _R.useState(true);
  const dragging = _R.useRef(false);
  const selPreset = PRESETS.find(p=>p.key===sel);
  const selCat = selPreset?.cat;
  const catColor = CAT_COLORS[selCat] || "var(--rust)";

  _R.useEffect(()=>{ setPos(52); setRawOk(true); }, [sel]);
  ...
  return (
    <div className="pf-mdc">
                  ...
                  {items.map((p,j)=>{
                    ...
                    return (
                      <button key={p.key}
                        ...
                        onClick={()=>pick(p.key)}
                        style={{position:"relative",width:"100%",appearance:"none",border:"none",cursor:"pointer",textAlign:"left",
                          display:"grid",gridTemplateColumns:"32px 1fr",alignItems:"center",gap:11,padding:"7px 8px 7px 16px",
                          background:on?"rgba(166,84,43,.10)":"transparent",borderLeft:on?"2px solid var(--rust)":"2px solid transparent",transition:"background .15s"}}>
      ...
      <div className="pf-mdc-preview"
        ...
        <img key={"t-"+sel} src={presetImg(sel)} alt={selPreset && selPreset.name} draggable={false} style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",pointerEvents:"none"}} />
...
// ────────────────────────────────────────────────────────────────
const HERO_REDESIGN_IMG = "public/images/presets/hero-laguna.jpg";
const HERO_REDESIGN_RAW = "public/images/presets/hero-laguna-raw.jpg";
function HeroRedesigned(){
  const [rawOk, setRawOk] = _R.useState(true);
  const secRef = _R.useRef(null);
  const interacting = _R.useRef(false);
  _R.useEffect(()=>{
    ...
    const tick=(t)=>{
      ...
      if(!interacting.current){
        ...
        const e=(1-Math.cos(phase*2*Math.PI))/2;        // ease in-out 0..1..0
  ...
  const setRev=(clientX)=>{ const sec=secRef.current; if(!sec) return; const r=sec.getBoundingClientRect(); const p=Math.max(4,Math.min(96,((clientX-r.left)/r.width)*100)); sec.style.setProperty("--rev",p+"%"); };
  return (
    <section className="pf-hero2" ref={secRef}
      ...
      <img className="pf-hero2-img" src={HERO_REDESIGN_IMG} alt="" draggable={false} />
      {rawOk ? <img className="pf-hero2-raw" src={HERO_REDESIGN_RAW} alt="" draggable={false} onError={()=>setRawOk(false)} /> : null}
      {rawOk ? (<_R.Fragment>
...
// ────────────────────────────────────────────────────────────────
// Quem sou eu — autor real (substitui depoimentos fabricados)
// ────────────────────────────────────────────────────────────────
    ...
    <section className="pf-about" style={{background:"var(--forest)",color:"var(--canvas)",borderBottom:"1px solid rgba(232,223,201,.1)"}}>
      <div className="pf-about-grid">
        <div className="pf-about-photo">
          <img src="public/images/henrique-portrait-1.jpg" alt="Henrique Sesana" draggable={false} />
        </div>
        <div className="pf-about-text">
          ...
          <div className="pf-about-stats">
            {[["5+","anos em campo"],["50+","destinos"],[TOTAL_PRESETS,"presets"]].map(([n,l])=>(
...
window.PresetsFotografiaPage = PresetsFotografiaPage;
